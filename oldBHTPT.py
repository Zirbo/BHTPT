...
class param:
    #IPC
    e_bb, e_bs, e_ss, ecc, sRadius, e_min = 7.2158,   -193.095,   1721.9,   0.3,   0.25,  1.7327
...
risult = open('TPT'+str(int(kT*100.)),'w')
risult.write(str("#rho").ljust(20)+'\t'+str("p").ljust(20)+'\t'+str("mu").ljust(20)+'\t'+str("betaF").ljust(20)+'\n')


print 'Potenziale calcolato'
for rho in rhoset:
    compute_state(rho,kT, A, Nr,dr,nNodes,phi,phisq,x)
    risult.write(repr(rho).ljust(20)+'\t'+repr(A.P).ljust(20)+'\t'+repr(A.mu).ljust(20)+'\t'+repr(A.betaF).ljust(20)+'\n')
